...
do
	...
	do
		...
		cgexec -g memory:cache-test-arghya ./executables/extmem-merge-sort-balloon $memory_given_run $data_size_run cache-test-arghya 1 8 8
		...
		cgexec -g memory:cache-test-arghya ./executables/extmem-merge-sort-balloon $memory_given_run $data_size_run cache-test-arghya 2 8 8
		pids=( $(pgrep -f balloon) )
		for pid in "${pids[@]}"; do
		  if [[ $pid != $$ ]]; then
		    kill "$pid"
		...
		#code to replay worse case memory profile from merge sort
