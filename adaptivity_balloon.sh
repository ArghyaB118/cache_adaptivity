...
declare -a data_size=( 256 512 1024 2048 ) 
declare -a memory_given=( 256 256 256 256 )
...
do
	...
	do
		...
		# for j in `seq 1 $NUMBALLOONS`;
		# do
		# 	cgexec -g memory:cache-test-arghya ./executables/balloon > "balloon_data/balloon_log$j.txt" 0 $TOTALMEMORY_MB $STARTINGMEMORY_MB $NUMBALLOONS $j &
		# done
		# cgexec -g memory:cache-test-arghya ./executables/extmem-merge-sort-balloon $memory_given_run $data_size_run cache-test-arghya 1 $fanout $block_size
		...
		#code to generate adaptive adversarial worse-case memory profile from merge sort
		...
		cgexec -g memory:cache-test-arghya ./executables/extmem-merge-sort-balloon $memory_given_run $data_size_run cache-test-arghya 2 $fanout $block_size
		pids=( $(pgrep -f balloon) )
		for pid in "${pids[@]}"; do
		if [[ $pid != $$ ]]; then
		 kill "$pid"
		...
		cp mem_profile.txt memory_profiles/worst_case$data_size_run.txt
		for j in `seq 1 $NUMBALLOONS`;
		do
		     rm balloon_data/balloon_log$j.txt
		done

		# #code to replay adaptive adversarial worse-case memory profile from merge sort
		...
		#run cache-adaptive on adaptive adversarial worst-case
		...
		# echo "Running merge sort to generate random memory"
		# ./cgroup_creation.sh cache-test-arghya
		# ./executables/make-unsorted-data $data_size_run
		# sudo sh -c "sync; echo 3 > /proc/sys/vm/drop_caches; echo 0 > /proc/sys/vm/vfs_cache_pressure"
		# sudo bash -c "echo 1 > /var/cgroups/cache-test-arghya/memory.oom_control"
		# echo "merge sort to generate random memory for data size $data_size_run and memory size $memory_given_run" >> out-sorting.txt
		# echo $TOTALMEMORY > /var/cgroups/cache-test-arghya/memory.limit_in_bytes
		# for j in `seq 1 $NUMBALLOONS`;
		# do
		#      cgexec -g memory:cache-test-arghya ./executables/balloon > "balloon_data/balloon_log$j.txt" 3 $TOTALMEMORY_MB $STARTINGMEMORY_MB $NUMBALLOONS $j &
		# done
		# cgexec -g memory:cache-test-arghya ./executables/extmem-merge-sort-balloon $memory_given_run $data_size_run cache-test-arghya 3 $fanout $block_size
		# pids=( $(pgrep -f balloon) )
		# for pid in "${pids[@]}"; do
		# if [[ $pid != $$ ]]; then
		#  kill "$pid"
		...
		# # #putting the important memory profile in a separate file before the two next sections
		# cp mem_profile.txt mem_profile_use.txt
		# cp mem_profile.txt memory_profiles/random$data_size_run.txt
		# for j in `seq 1 $NUMBALLOONS`;
		# do
		#      rm balloon_data/balloon_log$j.txt
		# done
		...
		# echo "Running merge sort to replay random memory"
		# ./cgroup_creation.sh cache-test-arghya
		# ./executables/make-unsorted-data $data_size_run
		# sudo sh -c "sync; echo 3 > /proc/sys/vm/drop_caches; echo 0 > /proc/sys/vm/vfs_cache_pressure"
		# sudo bash -c "echo 1 > /var/cgroups/cache-test-arghya/memory.oom_control"
		# echo "merge sort to replay random memory for data size $data_size_run and memory size $memory_given_run" >> out-sorting.txt
		# echo $TOTALMEMORY > /var/cgroups/cache-test-arghya/memory.limit_in_bytes
		# for j in `seq 1 $NUMBALLOONS`;
		# do
		#      cgexec -g memory:cache-test-arghya ./executables/balloon > "balloon_data/balloon_log$j.txt" 1 $TOTALMEMORY_MB $STARTINGMEMORY_MB $NUMBALLOONS $j &
		...
		# echo "Running funnel sort on random memory"
		# ./cgroup_creation.sh cache-test-arghya
		# ./executables/make-unsorted-data $data_size_run
		# sudo sh -c "sync; echo 3 > /proc/sys/vm/drop_caches; echo 0 > /proc/sys/vm/vfs_cache_pressure"
		# sudo bash -c "echo 1 > /var/cgroups/cache-test-arghya/memory.oom_control"
		# echo "funnel sort on random memory for data size $data_size_run and memory size $memory_given_run" >> out-sorting.txt
		# echo $TOTALMEMORY > /var/cgroups/cache-test-arghya/memory.limit_in_bytes
		# for j in `seq 1 $NUMBALLOONS`;
		# do
		#      cgexec -g memory:cache-test-arghya ./executables/balloon > "balloon_data/balloon_log$j.txt" 1 $TOTALMEMORY_MB $STARTINGMEMORY_MB $NUMBALLOONS $j &
		...
		#code to generate adaptive benevolent memory profile from merge sort
		echo "Running merge sort to generate adaptive benevolent memory"
		./cgroup_creation.sh cache-test-arghya
		./executables/make-unsorted-data $data_size_run
		sudo sh -c "sync; echo 3 > /proc/sys/vm/drop_caches; echo 0 > /proc/sys/vm/vfs_cache_pressure"
		sudo bash -c "echo 1 > /var/cgroups/cache-test-arghya/memory.oom_control"
		echo "merge sort to generate adaptive benevolent memory for data size $data_size_run and memory size $memory_given_run" >> out-sorting.txt
		echo $TOTALMEMORY > /var/cgroups/cache-test-arghya/memory.limit_in_bytes
		for j in `seq 1 $NUMBALLOONS`;
		do
		     cgexec -g memory:cache-test-arghya ./executables/balloon > "balloon_data/balloon_log$j.txt" 3 $TOTALMEMORY_MB $STARTINGMEMORY_MB $NUMBALLOONS $j &
		done
		cgexec -g memory:cache-test-arghya ./executables/extmem-merge-sort-balloon $memory_given_run $data_size_run cache-test-arghya 4 $fanout $block_size
		pids=( $(pgrep -f balloon) )
		for pid in "${pids[@]}"; do
		if [[ $pid != $$ ]]; then
		 kill "$pid"
		...
		# #putting the important memory profile in a separate file before the two next sections
		cp mem_profile.txt mem_profile_use.txt
		cp mem_profile.txt memory_profiles/benevolent$data_size_run.txt
		for j in `seq 1 $NUMBALLOONS`;
		do
		     rm balloon_data/balloon_log$j.txt
		done
		...
		# #code to replay random memory profile from merge sort
		# echo "Running merge sort to replay adaptive benevolent memory"
		# ./cgroup_creation.sh cache-test-arghya
		# ./executables/make-unsorted-data $data_size_run
		# sudo sh -c "sync; echo 3 > /proc/sys/vm/drop_caches; echo 0 > /proc/sys/vm/vfs_cache_pressure"
		# sudo bash -c "echo 1 > /var/cgroups/cache-test-arghya/memory.oom_control"
		# echo "merge sort to replay adaptive benevolent memory for data size $data_size_run and memory size $memory_given_run" >> out-sorting.txt
		# echo $TOTALMEMORY > /var/cgroups/cache-test-arghya/memory.limit_in_bytes
		# for j in `seq 1 $NUMBALLOONS`;
		# do
		#      cgexec -g memory:cache-test-arghya ./executables/balloon > "balloon_data/balloon_log$j.txt" 1 $TOTALMEMORY_MB $STARTINGMEMORY_MB $NUMBALLOONS $j &
		...
		#run cache-adaptive on random memory
		echo "Running funnel sort on adaptive benevolent memory"
		./cgroup_creation.sh cache-test-arghya
		./executables/make-unsorted-data $data_size_run
		sudo sh -c "sync; echo 3 > /proc/sys/vm/drop_caches; echo 0 > /proc/sys/vm/vfs_cache_pressure"
		sudo bash -c "echo 1 > /var/cgroups/cache-test-arghya/memory.oom_control"
		echo "funnel sort on adaptive benevolent memory for data size $data_size_run and memory size $memory_given_run" >> out-sorting.txt
		echo $TOTALMEMORY > /var/cgroups/cache-test-arghya/memory.limit_in_bytes
		for j in `seq 1 $NUMBALLOONS`;
		do
		     cgexec -g memory:cache-test-arghya ./executables/balloon > "balloon_data/balloon_log$j.txt" 1 $TOTALMEMORY_MB $STARTINGMEMORY_MB $NUMBALLOONS $j &
