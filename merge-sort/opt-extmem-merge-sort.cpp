...
int main(int argc, char *argv[]){
  ...
  if (k_logical == 0)
    k_logical = (memory_given_MB / 4) * 256; // memory_given_MB * 1024 * 1024 / (4 * 1024);
  else if (k_logical == 1)
  ...
  if (logical_block_size_MB == 0)
    base_case = 1048576ULL * memory_given_MB / sizeof(TYPE);
  else
  ...
  if ((k_logical + 1) * logical_block_size_MB > memory_given_MB) {
    cout << "constarint not met" << endl;
    return 0;
  }
  if (k_logical == 0 && logical_block_size_MB != 0) {
    cout << "for B = 4k and k = M / 4B, you can't choose logical block size" << endl;
    return 0;
  }
 	std::cout << "\n==================================================================\n";
